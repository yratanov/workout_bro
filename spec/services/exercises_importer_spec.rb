...
describe ExercisesImporter do
  ...
  describe '#call' do
    ...
    let(:temp_file) { Tempfile.new([ 'exercises', '.csv' ]) }

    before do
      temp_file.write(csv_content)
      temp_file.rewind
      allow_any_instance_of(described_class).to receive(:path_for_locale).and_return(temp_file.path)
    end
    ...
    it 'imports exercises from CSV' do
      expect { described_class.new(user: user).call }.to change(Exercise, :count).by(2)
    end
    ...
    context 'when exercise already exists for user' do
      before do
        user.exercises.create!(name: 'Test Exercise', with_weights: true, with_band: false)
      end

      it 'skips existing exercises' do
        expect { described_class.new(user: user).call }.to change(Exercise, :count).by(1)
      end
