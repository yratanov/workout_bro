...
RSpec.describe WorkoutImports::Parsers::HevyParser do
  ...
  describe "#parse" do
    context "with valid Hevy format" do
      ...
      it "creates workout reps with correct weight from weight_kg" do
        ...
        squat_set = jan15_workout.workout_sets.joins(:exercise)
                                 .where(exercises: { name: "Squat" }).first

        expect(squat_set.workout_reps.count).to eq(2)
        expect(squat_set.workout_reps.map(&:weight)).to contain_exactly(100.0, 110.0)
      end
    end
