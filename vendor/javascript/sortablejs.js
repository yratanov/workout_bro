...
Sortable.prototype = {
  ...
  _prepareDragStart: function _prepareDragStart(e, r, i) {
    ...
    if (i && !v && i.parentNode === s) {
      ...
      on(u, "touchmove", ae);
      if (c.supportPointer) {
        on(u, "pointerup", l._onDrop);
        !this.nativeDraggable && on(u, "pointercancel", l._onDrop);
      } else {
      ...
      if (
        ...
        if (c.supportPointer) {
          on(u, "pointerup", l._disableDelayedDrag);
          on(u, "pointercancel", l._disableDelayedDrag);
        } else {
  ...
  _disableDelayedDragEvents: function _disableDelayedDragEvents() {
    ...
    off(e, "pointerup", this._disableDelayedDrag);
    off(e, "pointercancel", this._disableDelayedDrag);
  ...
  _offUpEvents: function _offUpEvents() {
    ...
    off(e, "pointercancel", this._onDrop);
      ...
      if (t) {
              ...
              ~f &&
                ...
                (function () {
                  ...
                  for (; i < e; i++)
                    if (
                      !~De.indexOf(d[i]) &&
                      closest(d[i], c.draggable, o, false)
                    ) {
                      var l =
                        a &&
                        (typeof a === "function"
                          ...
                          : a.split(",").some(function (e) {
                              return closest(d[i], e.trim(), o, false);
                            }));
                      if (!l) {
                        toggleClass(d[i], c.selectedClass, true);
                        De.push(d[i]);
                        dispatchEvent({
                          sortable: r,
                          rootEl: n,
        ...
        if (Te && this.isMultiDrag) {
          ...
          if ((o[u].options.sort || o !== n) && De.length > 1) {
            var g = getRect(ye),
              p = index(ye, ":not(." + this.options.selectedClass + ")");
            ...
            if (!xe) {
              ...
              removeMultiDragElements();
              De.forEach(function (e) {
                d[p] ? o.insertBefore(e, d[p]) : o.appendChild(e);
                p++;
              });
              if (a === index(ye)) {
                var v = false;
                De.forEach(function (e) {
                  e.sortableIndex === index(e) || (v = true);
                });
                if (v) {
                  i("update");
                  i("sort");
